    ...
    <div className={styles.show_days}>
      ...
      <div className={styles.dates}>
        {showDates.map((each, index) => {
          ...
          const date: string = `${tagetday.year}-${String(tagetday.month).padStart(2, "0")}-${String(each).padStart(2, "0")}`;
          if (each > index || (index - each) > 7) {
            classNameTemp = `${styles.date} ${styles.other}`
          }
          return <div className={classNameTemp} key={index}>
            <Link to={`/book/${date}`} style={{ color: "inherit", textDecoration: "inherit" }}>{each}</Link></div>;
        })}
      </div>
    </div>
  </div>;
